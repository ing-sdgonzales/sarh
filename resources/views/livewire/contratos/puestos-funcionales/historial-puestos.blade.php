        ...
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <div class="mt-2 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 mb-2">
                <div class="sm:col-span-6 mx-auto">
                    ...
                    <x-label
                        value="{{ $empleado->codigo . ' - ' . $empleado->nombres . ' ' . $empleado->apellidos }}" />
                    {{-- <label class="mt-1 block text-sm font-medium leading-6 text-gray-900">{{ $empleado->codigo }} -
                        {{ $empleado->nombres }}
                        {{ $empleado->apellidos }}</label> --}}
                    <x-label class="mt-1 block text-center" value="{{ $empleado->dpi }}" />
                    {{-- <label
                        ...
                        <tbody class="bg-white dark:bg-gray-600">
                            @foreach ($registros_puestos as $registro)
                                <tr
                                    ...
                                    <td
                                        class="py-2 px-4 {{ $loop->last ? 'border-none' : 'border-b border-gray-200 dark:border-gray-700' }}">
                                        {{ $registro->numero_contrato }}</td>
                                    <td
                                        class="py-2 px-4 {{ $loop->last ? 'border-none' : 'border-b border-gray-200 dark:border-gray-700' }}">
                                        {{ $registro->puesto_funcional }}</td>
                                    <td
                                        class="py-2 px-4 {{ $loop->last ? 'border-none' : 'border-b border-gray-200 dark:border-gray-700' }}">
                                        {{ $registro->dependencia_funcional }}</td>
                                    <td
                                        class="py-2 px-4 {{ $loop->last ? 'border-none' : 'border-b border-gray-200 dark:border-gray-700' }}">
                                        {{ date('d/m/Y', strtotime($registro->fecha_inicio)) }} -
                                        {{ date('d/m/Y', strtotime($registro->fecha_fin)) }}
                                    </td>
                                    <td
                                        class="py-2 px-1 {{ $loop->last ? 'border-none' : 'border-b border-gray-200 dark:border-gray-700' }}">
