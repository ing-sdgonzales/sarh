    ...
    <div class="flex justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-gray-100 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full"
            ...
            <div
                ...
                <!--Close button-->
                <button type="button" wire:click='cerrarModalValidacion'
            ...
            <!--Modal body-->
            <form method="POST" wire:submit='guardarValidacion'>
                @method('POST')
                @csrf
                            ...
                            <div class="mt-2 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                <div class="sm:col-span-full">
                                    <p class="text-justify">
                                        Solicitud de vacaciones del empleado <b>{{ $nombre_empleado }}</b> para el
                                        período del <b>{{ date('d/m/Y', strtotime($fecha_salida)) }}</b> al
                                        <b>{{ date('d/m/Y', strtotime($fecha_ingreso . '-1 day')) }}</b> con duración de
                                        <b>{{ $duracion . ' día(s) hábiles' }}</b>. El empleado deberá reincorporarse a
                                        sus labores el día <b>{{ date('d/m/Y', strtotime($fecha_ingreso)) }}</b>.
                                    </p>
                                </div>
                                <div class="sm:col-span-full">
                                    <label for="aprobada"
                                    ...
                                    <div class="mt-2">
                                        <select wire:model='aprobada' id="aprobada" name="aprobada" required
                                            class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
                                            <option value="">Seleccionar...</option>
                                            @foreach ($validacion ?? [] as $validacion)
                                                <option value="{{ $validacion['val'] }}">
                                                    {{ $validacion['res'] }}
                                                </option>
                                            @endforeach
                                        </select>
